...
#### Experimental Intel Mac Radeon support via MoltenVK
> [!WARNING]
> This build path is experimental. No official support is provided.

This repo includes an experimental Vulkan backend (via MoltenVK) aimed at **Intel Macs with AMD/Radeon GPUs**.
...
1. Install dependencies (once per machine):
   ...
   ```
   > Also install the standard dev prerequisites first (Go, Node.js/npm, Xcode command line tools, etc.).

2. Build universal binaries and app bundles (output goes to `dist/`):
   ```bash
   ./scripts/build_darwin_vulkan.sh
   ```

The resulting `dist/Ollama.app` is the standard app bundle with the MoltenVK backend enabled.
It may run on both Apple Silicon and Intel Macs, but it is optimized for Intel + Radeon and will usually be slower on Apple Silicon.
